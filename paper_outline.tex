...
痰湿质 surprisingly 排名靠后（$\beta^*=-0.008$，$\text{OR}=0.992$，$p=0.919$），这一反直觉现象的可能原因在于：\textbf{体质标签（主体质）与体质积分高度耦合}，当主体质已被标签变量部分解释后，痰湿积分的边际贡献被压缩；此外，样例数据中体质积分与血脂指标的线性关联整体偏弱，也可能导致回归系数不稳定。进一步计算VIF值，发现气虚质与平和质的VIF分别为$4.2$和$3.8$，虽未超过5的常用阈值，但已接近警戒线。若后续研究能获取更大样本或补充痰湿体质专属生化标志物（如脂联素、炎症因子），有望提升模型的统计效能。
...
\midrule
气虚质  & 0.123 & 1.130 & 0.124 & 4.2 \\
平和质  & 0.083 & 1.087 & 0.340 & 3.8 \\
湿热质  & $-0.051$ & 0.951 & 0.524 & 2.1 \\
血瘀质  & $-0.048$ & 0.954 & 0.550 & 2.3 \\
阳虚质  & 0.021 & 1.021 & 0.796 & 1.8 \\
痰湿质  & $-0.008$ & 0.992 & 0.919 & 2.5 \\
\bottomrule
\multicolumn{5}{l}{\small 注：剩余三种体质（气郁质、特禀质、阴虚质）系数绝对值均$<0.02$，$p>0.85$，从略。}
\end{tabular}
\end{table}
...
本论文的全部源代码使用 Python 3.12 编写，保存为文件 \texttt{solution.py}，随支撑材料一并提交。程序涵盖数据读取、预处理、问题1（相关性分析+组间差异检验+逐步Logistic回归+九种体质贡献度分析）、问题2（风险评分构造+阈值划分+LDA验证）、问题3（穷举优化+结果汇总）的完整流程。下面给出问题3中\textbf{穷举优化算法}的核心代码片段，其余模块详见支撑材料。
